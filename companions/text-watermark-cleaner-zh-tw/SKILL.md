...
不要把「去 AI 味」和「去文字浮水印」當成同一個動作。只有使用者明確要求浮水印或 provenance 清理時才執行本 skill；單純要求文字更自然時，交給譯神（`awesome-eason`）的「說人話」模式。

本 skill 以譯神選配 companion 形式存在於 `companions/text-watermark-cleaner-zh-tw/`；也可被安裝成獨立 skill。來源與授權見 `NOTICE.md`。

## 模式選擇

- **inspect**：只檢查並報告，不修改內容。
- **layer-a**：只做不可見文字清理，預設保守模式。譯神預設只做到這層。
- **layer-b**：只做一次統計式改寫；需明確同意可能的語意與語氣漂移。best-effort，不可宣稱已過官方偵測。
- **full**：`inspect → Layer A → Layer B（可選）→ Layer A → after inspect`。

如果使用者同時要求去 AI 味與去浮水印，採用：

```text
保護非 prose 區段 → inspect → Layer A → 譯神說人話 → 可選 Layer B → Layer A → after inspect
```
...
本技能優先使用內附的本地 deterministic scripts；它們是從 [`guillaumemeyer/watermarks-remover`](https://github.com/guillaumemeyer/watermarks-remover) 的 `service/scripts` 擷取，並保留上游 MIT 授權。

macOS / Linux：

```bash
skillRoot="companions/text-watermark-cleaner-zh-tw"
bash "$skillRoot/scripts/run-text-watermark.sh" inspect ./article.md --json
```

Windows PowerShell：

```powershell
$skillRoot = (Resolve-Path .\companions\text-watermark-cleaner-zh-tw).Path
& "$skillRoot\scripts\run-text-watermark.ps1" `
  -Mode Inspect -InputPath .\article.md -Json
```

需要較嚴格的檢查時才加上 `--aggressive` / `-Aggressive` 或 `--strip-emoji-glue` / `-StripEmojiGlue`。若要同時取得統計式報告，可加 `--stylometry` / `-Stylometry`；這是分析訊號，不是 Claude 官方 detector。
...
使用內附 `scripts/run-text-watermark.sh clean`（或 Windows 的 `run-text-watermark.ps1 -Mode Clean`）、服務的 `/clean` 或等效 deterministic cleaner，並保留原檔，預設產生 `*.cleaned.*`。本地入口範例：

```bash
bash "$skillRoot/scripts/run-text-watermark.sh" clean ./article.md --stats
```
...
- 「此外」「值得注意的是」「在……背景下」「這不僅……而且……」等屬於語氣修訂範圍，不是 Layer A 的證據；需要時交給譯神「說人話」，不要把它們當作已偵測到 watermark。
...
- `scripts/run-text-watermark.ps1` 是 Windows PowerShell 入口；`scripts/run-text-watermark.sh` 是 macOS／Linux 入口。兩者都走 UTF-8、保守清理選項與 `.cleaned` 輸出路徑。
...
- 「先去浮水印，再幫我去 AI 味。」→ `full`，並依序執行保護、Layer A、譯神說人話、可選 Layer B、Layer A。
